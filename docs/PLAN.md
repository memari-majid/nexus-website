...
| **Pages** | Live 2026-09-13 | `/` · `/about` (team index) · `/about/<slug>` ×3 (per-person, one shared template from `lib/people.ts`) · `/nvidia-dli-workshops` · `/contact` · `/how-it-works` (public teardown of the AI Consultant; same URL, retitled 2026-09-13, and now also the home of the evaluations view). All in `sitemap.xml`. Nav is exactly four links in this order: **Consulting · Training · About · Contact**, plus the persistent **"Start a conversation"** CTA (desktop and mobile) that opens the chat. |
| **Design** | Sleek pass 2026-09-12 · demo section 2026-09-13 | Apple-like: one idea per section, generous space, short declarative copy, **no trailing periods in headlines**. Homepage = hero + Consulting + **Try our AI** + Training + Team + footer. "Try our AI" (added 2026-09-13) holds the inline AI Consultant demo and is the one sanctioned addition to the homepage inventory: rule and cost posture in [`AGENTS.md`](../AGENTS.md) §9.3. Depth (workshop outline, NVIDIA verification links, collaborations) lives on inner pages, not the homepage. Removed: stats grid, partners strip, AI-now, portfolio, careers, FAQ accordion, market widgets, `/api/news`, `/api/market`. |
| **Inline AI demo** | Live 2026-09-13 | The assistant is presented as a product, not only a bubble: the homepage "Try our AI" section embeds a working AI Consultant chat in the normal page flow, with the floating launcher still covering the rest of the site. Fixed height on desktop (about 560 to 640 px), fills the section column, scrolls internally, never grows the page. Both shells share one conversation through `app/components/chat/chatStore.ts`, so a visitor who opens the floating panel keeps the transcript. The demo seeds the cheaper model to keep a first tap inexpensive; the picker still offers all four. Every turn goes through the same limiter, budgets, and `chat.usage` log line as the widget. |
| **Evaluations** | Live 2026-09-13 | Visible proof that model choice is measured. The checked-in artifact `evals/results/bakeoff-latest.json` (schema in `lib/evals.ts`, covered by `lib/evals.test.ts`) renders as the published table on `/how-it-works`; a visitor can also run two picker models live against one prompt at `app/api/evals/run/route.ts` (NDJSON progress frames, `maxDuration` 120 in `vercel.json`, contestants and judge in two bounded legs). The judge is never one of the contestants. One run per visitor per UTC day, counted when the run is reserved rather than when it finishes, so a visitor who closes the tab has still spent it; a repeat of the same model pair replays from cache. Spend is capped by `GLOBAL_EVAL_USD_PER_DAY` inside the soft budget and still passes `reserveBudget`, the only hard ceiling, and an abandoned run settles upward only (never refunded below the reservation, because the calls were billed). A live block is labelled with its own shape and never rewrites a published cell. Without a shared store (Upstash) the button refuses and spends nothing. |
...
1. **Shared usage-log helper.** `/api/chat` and `/api/evals/run` each build their own
   `chat.usage` style JSON line. Extracting one helper was deliberately deferred while the
   two routes settled (see the change plan §4.1): the limiter call order differs between
   them and a premature shared helper would hide that. Do it when a third caller appears,
   and keep the field names stable so existing Vercel log filters keep working.
2. **Model default review.** The demo and the floating panel are one conversation with one
   model pick (`chatStore.ts` seeds `FALLBACK_MODEL_ID`), so both open on the cheaper model.
   `AI_CHAT_MODEL` is now only the server-side default for a request that names no model.
   Revisit once there is a week of `chat.usage` data showing what visitors actually switch to.
3. **Republish the bake-off** after any prompt change: the published `promptHash` is what
   makes the table honest. **Open now:** the checked-in `evals/results/bakeoff-latest.json` was
   assembled by hand from a run that predates the `--json` writer. Its `promptHash` is `null`,
   its `shape.promptCall` is `nexusChatSystem()` while `evals/bakeoff.ts` now renders
   `nexusChatSystem({ emailEnabled: false })` to match production, every `rubricScores` is
   `null`, and the chat prompt has changed since (the founder naming rule is a parameter now).
   The file is honest about all of that in its own `notes`, which is why it may stand for the
   moment, but it is a hand-assembled artifact in a section whose rule is that a score exists
   only if someone measured it. Regenerate it with `npx tsx evals/bakeoff.ts --json ...` and
   then `npx tsx evals/judge.ts`. Both spend real money, so it is an owner-run step; until it
   happens the page keeps saying no scored run is published rather than showing a number.
4. **Assistant naming is a prompt parameter now.** `nexusAssistantSystem("chat" | "site")`
...
5. **Persona parity.** `lib/chat-persona.ts` and `../majidmemari/lib/chat-persona.ts` export the
   same SHARED NAMES block, same order. The personal site still renders its badge twice (widget
   header and inline demo) where this repo renders it once inside `ConversationView`; folding
   that into its `ConversationView` is the remaining parity edit, and it is cosmetic.
6. **Optional: port `/how-it-works` to the personal site.** Sanctioned either way by
   `AGENTS.md` §9.2: that site's teardown surface is the demo's "What it does" tab today. A port
   touches `app/how-it-works/page.tsx`, `lib/seo.ts` (`INDEXABLE_PATHS`, append-only), the widget
   toolbar and the demo header in `../majidmemari`.
